...
const onvifServer = require('./src/onvif-server-no-broadcast');
...
const parser = new argparse.ArgumentParser({
    description: 'Virtual Onvif Server (No Broadcast)'
});
    ...
    if (args.create_config) {
        ...
        for (let onvifConfig of config.onvif) {
            ...
            if (server.getHostname()) {
                ...
                server.startServer();
                // Discovery service is disabled in this version
                logger.info(`Note: ONVIF discovery broadcast is disabled in this version`);
                if (args.debug)
